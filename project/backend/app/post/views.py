    ...
    if request.method == 'POST':
        ...
        for i in range(repeating_frequency):
            ...
            event = {"post_name": post_name, "owner": actor_id, "sport_category": sport_id, \
                     ...
                     "longitude": longitude, "latitude": latitude, "date_time": current_event_date,
                     "participant_limit": participant_limit, "spectator_limit": spectator_limit, \
                     "rule": rule, "equipment_requirement": equipment_requirement, "status": event_status,
                     "capacity": capacity, \
            ...
            if event_ser.is_valid():
                ...
                event_act_stream_ser = EventPostActivityStreamSerializer(
...
def createEquipmentPost(request):
    if request.method == 'POST':
        ...
        created_date = datetime.datetime.now()
        image = ""
        equipment_post_ser = EquipmentPostSerializer(
            data={"post_name": equipment_post_name, "owner": owner_id, "sport_category": sport_id,
                  "created_date": created_date + datetime.timedelta(hours=3), \
                  "description": description, "longitude": longitude, "latitude": latitude, "link": link,
                  "active": active, "pathToEquipmentPostImage": image})
        ...
        if equipment_post_ser.is_valid():
            ...
            return Response({"message": "there was an error while deleting the equipment post"},
                            status=status.HTTP_406_NOT_ACCEPTABLE)

        equipment_post_act_stream_ser = EquipmentPostActivityStreamSerializer(
            data={"context": data["@context"], "summary": data["summary"], \
        ...
        result = {"@context": data["@context"], "summary": data["summary"], "type": data["type"],
                  "actor": data["actor"], "object": res}
...
def deleteEquipmentPost(request):
    ...
    equipment_post_ser = EquipmentPostActivityStreamSerializer(
        data={"context": data["@context"], "summary": data["summary"], \
...
def deleteEventPost(request):
    ...
    if event_post_ser.is_valid():
        ...
        return Response({"message": "there was an error while deleting the event post"},
                        status=status.HTTP_406_NOT_ACCEPTABLE)
...
def applyToEvent(request):
    ...
    if not (
            event_post.capacity == "open to applications" and event_post.status == "upcoming"):  # if the event is not open to applications or event status is
        return Response({"message": "Event is closed to applications"},
                        status=400)  # event is full, passed or cancelled       HTTP412 maybe??
    ...
    application_act_stream_ser = EventPostActivityStreamSerializer(
        data={"context": data["@context"], "summary": data["summary"], "type": data["type"], "actor": actor_id,
...
def acceptApplicant(request):
    ...
    try:
        ...
        return Response({"message": "There is no such application in the database, operation is aborted"},
                        status=status.HTTP_404_NOT_FOUND)
    ...
    if event_post.participant_limit > event_post.current_player and event_post.status == "upcoming" and event_post.capacity == "open to applications":
        ...
        return Response(
            {"message": "The event is not available to accept the application. Full, cancelled or maybe past"},
            status=status.HTTP_400_BAD_REQUEST)

    application = application.update(status="accepted")

    accept_act_stream_ser = EventPostActivityStreamSerializer(
        data={"context": data["@context"], "summary": data["summary"], "type": data["type"], "actor": applicant_id,
              "object": data["object"]["Id"]})
...
def changeEquipmentInfo(request):
    ...
    equipment_post_act_ser = EquipmentPostActivityStreamSerializer(
        data={"context": data["@context"], "summary": data["summary"], \
    ...
    if equipment_post_act_ser.is_valid():
        ...
        if "sport_category" in modifications_keys:
            try:
                ...
                sport_ser = SportSerializer(
                    data={"sport_name": data["modifications"]["sport_category"], "is_custom": True})
                if sport_ser.is_valid():
                    s = sport_ser.save()
                    data["modifications"]["sport_category"] = s.id
                # Sport name has too many caharacters
        ...
        if equipment_post_ser.is_valid():
            ...
            equipment_post_updated["sport_category"] = Sport.objects.get(
                id=equipment_post_updated["sport_category"]).sport_name
            equipment_post_act_ser.save()
        else:
            return Response({"message": equipment_post_ser.errors}, 422)

    else:
        return Response({"message": "There was an error while updating the equipment post"},
                        status=status.HTTP_406_NOT_ACCEPTABLE)
    ...
    res = {"@context": data["@context"], "summary": data["summary"], "actor": data["actor"], "type": data["type"],
           "object": equipment_post_updated}
...
def changeEventInfo(request):
    ...
    if event_post_act_ser.is_valid():
        ...
        if event_post_ser.is_valid():
            ...
            event_post_updated["skill_requirement"] = SkillLevel.objects.get(
                id=event_post_updated["skill_requirement"]).level_name
        ...
        return Response({"message": "There was an error while updating the event post"},
                        status=status.HTTP_406_NOT_ACCEPTABLE)
...
def postponeEvent(request):
    ...
    event_post_act_ser = EventPostActivityStreamSerializer(
        data={"context": data["@context"], "summary": data["summary"], "actor": actor_id, "type": data["type"],
              "object": event_post.id})
    if event_post_act_ser.is_valid():
        event_post_act_ser.save()
    else:
...
def getWaitingApplications(request):
    ...
    applicationList = list(
        Application.objects.filter(status="waiting", applicant_type="player", event_post_id=eventId).values())
    if applicationList != []:
...
def getRejectedApplications(request):
    eventId = request.query_params["eventId"]
    applicationList = list(
        Application.objects.filter(status="rejected", applicant_type="player", event_post_id=eventId).values())
    if applicationList != []:
        return JsonResponse(applicationList, safe=False)
    else:
...
def getAcceptedApplications(request):
    eventId = request.query_params["eventId"]
    applicationList = list(
        Application.objects.filter(status="accepted", applicant_type="player", event_post_id=eventId).values())
    if applicationList != []:
        return JsonResponse(applicationList, safe=False)
    else:
...
def getInadequateApplications(request):
    eventId = request.query_params["eventId"]
    applicationList = list(
        Application.objects.filter(status="inadeq", applicant_type="player", event_post_id=eventId).values())
    if applicationList != []:
        return JsonResponse(applicationList, safe=False)
    else:
...
def getEventPostDetails(request):
    ...
    try:
        comments = list(
            EventComment.objects.filter(event_post_id=post_id).order_by('id').values('id', 'content', 'created_date',
                                                                                     'owner_id__username'))
    ...
    try:
        accepted_players = list(
            Application.objects.filter(event_post=post_id, status="accepted", applicant_type="player").values(
                'user__Id', \
    ...
    try:
        spectators = list(
            Application.objects.filter(event_post=post_id, status="accepted", applicant_type="spectator").values(
                'user__Id', \
                'user__name', 'user__surname', 'user__username'))
    except:
        spectators = []
    ...
    if actor_id == event_post_details.owner_id:
        try:
            waiting_players = list(
                Application.objects.filter(event_post=post_id, status="waiting", applicant_type="player").order_by(
                    'id').values('user__Id', \
                                 'user__name', 'user__surname', 'user__username'))
        except:
            waiting_players = []
        try:
            rejected_players = list(
                Application.objects.filter(event_post=post_id, status="rejected", applicant_type="player").values(
                    'user__Id', \
                    'user__name', 'user__surname', 'user__username'))
        except:
            rejected_players = []
        try:
            inadequate_player_applications = list(
                Application.objects.filter(event_post=post_id, status="inadequate", applicant_type="player").values(
                    'user__Id', \
                    'user__name', 'user__surname', 'user__username'))
        except:
            inadequate_player_applications = []
    ...
    act_str = EventPostActivityStreamSerializer(data={"context": data["@context"], "summary": data["summary"], \
                                                      "type": data["type"], "actor": actor_id,
                                                      "object": event_ser.data["id"]})

    if act_str.is_valid():
        act_str.save()
    else:
        return Response({"message": "there was an error while viewing the event post"},
                        status=status.HTTP_406_NOT_ACCEPTABLE)
    ...
    event_post_details["owner"] = \
    list(User.objects.filter(Id=event_post_details["owner"]).values('Id', 'name', 'surname', 'username'))[0]
...
def getEventPostAnalytics(request):
    ...
    if len(accepted_users_ids) == 0:
        ...
        for i in range(len(accepted_users_ids)):
            ...
            total_applied_event_count += len(
                list(Application.objects.filter(user=user_id, status='accepted', applicant_type='player')))
...
def getEquipmentPostDetails(request):
    ...
    try:
        comments = list(
            EquipmentComment.objects.filter(equipment_post_id=post_id).order_by('id').values('id', 'content', 'created_date', 'owner_id__username'))
        for i in range(len(comments)):
            comments[i]["created_date"] = comments[i]["created_date"].strftime('%d/%m/%Y %H:%M')
            comments[i]["username"] = comments[i].pop("owner_id__username")
    except:
    ...
    act_str = EquipmentPostActivityStreamSerializer(data={"context": data["@context"], "summary": data["summary"], \
                                                          "actor": actor_id, "type": data["type"],
                                                          "object": equipment_post_ser.data["id"]})
    if act_str.is_valid():
        act_str.save()
    else:
        return Response({"message": "there was an error while viewing the equipment post"},
                        status=status.HTTP_406_NOT_ACCEPTABLE)
    ...
    equipment_post_details["owner"] = \
    list(User.objects.filter(Id=equipment_post_details["owner"]).values('Id', 'name', 'surname', 'username'))[0]
    res = {"@context": data["@context"], "summary": data["summary"], "type": data["type"], "actor": data["actor"],
           "object": equipment_post_details}
...
def spectateToEvent(request):
    ...
    application_ser = ApplicationSerializer(
        data={"user": actor_id, "event_post": event_post.id, "status": applicationStatus,
              "applicant_type": "spectator"})
...
def createEventComment(request):
    ...
    comment_ser = EventCommentSerializer(data={"content": data["object"]["content"], "owner": user_id,
                                               "created_date": created_date + datetime.timedelta(hours=3),
                                               "event_post": event_post.id})
    if comment_ser.is_valid():
        comment_ser.save()
    else:
        return Response({"message": "There has been an error on comment creation"}, 400)

    event_comment_act_stream_ser = EventCommentActivityStreamSerializer(
        data={"context": data["@context"], "summary": data["summary"], "type": data["type"], "actor": user_id,
              "object": comment_ser.data["id"]})
...
def createEquipmentComment(request):
    ...
    comment_ser = EquipmentCommentSerializer(data={"content": data["object"]["content"], "owner": user_id,
                                                   "created_date": created_date + datetime.timedelta(hours=3),
                                                   "equipment_post": equipment_post.id})
    if comment_ser.is_valid():
        comment_ser.save()
    else:
        return Response({"message": "There has been an error on comment creation"}, 400)

    equipment_comment_act_stream_ser = EquipmentCommentActivityStreamSerializer(
        data={"context": data["@context"], "summary": data["summary"], "type": data["type"], "actor": user_id,
              "object": comment_ser.data["id"]})
...
class SaveBadgesScript(APIView):
    ...
    def post(self, request):
        badges = [
            {"name": "enthusiastic", "description": "the trait of being overly enthusiastic", "wikiId": "Q107261265"}, \
            {"name": "friendly", "description": "relationship between people who have mutual affection for each other",
             "wikiId": "Q491"}, \
            {"name": "leader",
             "description": "someone with the authority to affect the conduct of others; who have the responsibility of leading",
             "wikiId": "Q1251441"}, \
            {"name": "gifted", "description": "intellectual ability significantly higher than average",
             "wikiId": "Q467677"}, \
