...
def get_pollution(request):
    if request.method == 'GET':
        ...
        if 'lat' in request.GET and 'lon' in request.GET:
            url = "http://api.airvisual.com/v2/nearest_city?lat={}&lon={}&key={}".format(request.GET['lat'], request.GET['lon'], settings.POLLUTION_KEY)
            ...
            url = "http://api.airvisual.com/v2/nearest_city?lat={}&lon={}&key={}".format(locations.first().latitude, locations.first().longitude, settings.POLLUTION_KEY)
        else:
