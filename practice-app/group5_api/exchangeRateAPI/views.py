...
def index(request):
    return render(request, "exchangeRateAPI/index.html")
...
def get_category(request, category_name):
    if request.method == 'GET':
        try:
            ...
            response = json.dumps(
                [{'Category': category.name, 'Description': category.description, 'Max Player': category.max_player}])
        except:
...
def add_category(request):
    if request.method == 'POST':
        ...
        category = Category(
            name=category_name, description=category_description, max_player=max_player)
        try:
            category.save()
...
def add_event_post(request):
    if request.method == 'POST':
        ...
        post = EventPost(event_post_id=event_post_id, date_time=date_time, participation_limit=participation_limit, spectator_limit=spectator_limit, rule=rule,
                         equipment_requirement=equipment_requirement, status=status, location=location, contact_info=contact_info, skill_requirement=skill_requirement, repeating_frequency=repeating_frequency)
        try:
            post.save()
            response = json.dumps([{'Success': 'Event Post added.'}])
        except:
            response = json.dumps(
                [{'Error': 'Event Post could not be added.'}])
    return HttpResponse(response, content_type='text/json')
    ...
    if request.method == 'GET':
        try:
            ...
            response = requests.get(
                'https://openexchangerates.org/api/latest.json?app_id=9f85dc28cb9e43eb99dce0c859d49fc2')
...
def add_current_currency(request):
    if request.method == 'POST':
        ...
        currency = CurrentCurrency(base_currency=base_currency, base_total=base_total,
                                   target_currency=target_currency, target_result=target_result)
        list = []
        try:
            currency.save()
            result = str(base_total) + ' ' + base_currency + ' = ' + \
                str("{:.2f}".format(target_result)) + ' ' + target_currency
            response = json.dumps(
                [{'Success': 'Currenct currency added.', 'Result': result}])
        except:
            response = json.dumps(
                [{'Error': 'Currenct currency could not be added.'}])
    return HttpResponse(response, content_type='text/json')
